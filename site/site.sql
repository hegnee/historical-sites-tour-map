...
CREATE TABLE `sites` (
    `id` VARCHAR(255) NOT NULL COMMENT 'Unique identifier for the site (e.g., UUID)',
    `name` VARCHAR(255) NOT NULL COMMENT 'Name of the site',
    `description` TEXT DEFAULT NULL COMMENT 'Detailed description of the site',
    `category` VARCHAR(100) DEFAULT '' COMMENT 'Category of the site (e.g., Museum, Monument, Landmark)',
    `region` VARCHAR(100) DEFAULT '' COMMENT 'Geographical area or city where the site is located',
    `historical_period` VARCHAR(100) DEFAULT '' COMMENT 'Historical period the site belongs to (e.g., Ming Dynasty, Roman Empire)',
    `latitude` DECIMAL(10, 8) DEFAULT 0.0 COMMENT 'Latitude coordinate of the site',
    `longitude` DECIMAL(11, 8) DEFAULT 0.0 COMMENT 'Longitude coordinate of the site',
    `address` VARCHAR(255) DEFAULT '' COMMENT 'Full address of the site',
...
CREATE TABLE `multimedia` (
    `id` VARCHAR(255) NOT NULL COMMENT 'Unique identifier for the multimedia resource (e.g., UUID)',
    ...
    `description` VARCHAR(500) DEFAULT '' COMMENT 'Description of the multimedia resource',
    `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Timestamp of when the record was created',
    `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'Timestamp of when the record was last updated',
...
CREATE TABLE `dynamic_info` (
    `id` VARCHAR(255) NOT NULL COMMENT 'Unique identifier for the dynamic info record (e.g., UUID)',
    `site_id` VARCHAR(255) NOT NULL UNIQUE COMMENT 'Foreign key referencing the sites table, unique as one dynamic info per site',
    `opening_hours` JSON DEFAULT NULL COMMENT 'Structured opening hours (e.g., {"weekday": "9am-5pm", "weekend": "10am-4pm"})',
    `ticket_price` VARCHAR(255) DEFAULT '' COMMENT 'Ticket price information (e.g., "Adult: $20, Child: $10")',
    `notices` JSON DEFAULT NULL COMMENT 'Array of important notices or announcements (e.g., ["Closed for renovation", "Special event this weekend"])',
